...
export const drawLineChart = (data, windowWidth) => {
  ...
  const updateTooltipsVisibility = () => {
    d3.selectAll(".breakEvenLabel.tooltip").style(
      "display",
      `${shouldHideBep ? "none" : "block"}`
    );
  ...
  const initialSetUp = () => {
    const svg = d3
      ...
      .attr(
        "aria-label",
        `This image is a line graph representation of the break even point at ${data.breakEvenPoint.data[0].x} Units Sold and the data table below`
      );
  ...
  const setUpInteractiveTooltip = () => {
    ...
    mouseG = g.append("g").attr("class", "mouse-over-effects");
      ...
      .on("click mousemove touchmove focus", function () {
        ...
        d3.selectAll(".mouse-per-line").attr("transform", function (d, i) {
          if (becLines[i]) {
            ...
            obj.push({
              cost: Math.floor(y.invert(pos.y).toFixed(2)),
  ...
  const updateTooltipContent = (obj, unit) => {
    ...
    let tooltipClass =
      data["breakEven"].data[1].x < unit ? "tooltipLeft" : "tooltipRight";
    d3.select("#tooltip")
      .html("Units: " + unit)
  ...
  const detectClickOutside = () => {
    ...
    const insideGraph = d3.selectAll(
      ".graphCanvas, .graphCanvas *, .bec, .bec *, .becLine, #breakEvenCircle, #breakEvenCircle *"
    );
    ...
    d3.select("body").on("click", () => {
      ...
      if (outside) {
        shouldHideBep = false;
        updateTooltipsVisibility();
        d3.select("#lineChart").style("cursor", "pointer");
      }
    });
  };
